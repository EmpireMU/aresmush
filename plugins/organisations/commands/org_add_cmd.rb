      ...
      def handle
        ...
        ClassTargetFinder.with_a_character(self.target, client, enactor) do |model|
          if (model.in_organisation?(org['name']))
            client.emit_failure t('organisations.already_member', :name => model.name, :org => org['name'])
            return
          end
          
          model.add_to_organisation(org['name'])
          client.emit_success t('organisations.added', :name => model.name, :org => org['name'])
        end
      end
    end
  end
